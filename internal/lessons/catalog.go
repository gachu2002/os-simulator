		...
		Stages: []Stage{
			{
				...
				Objective:        "Identify dispatch and compute ordering under a deterministic scheduler.",
				Prompt:           "Run the workload and capture the scheduling timeline before making tuning choices.",
				Difficulty:       "intro",
				EstimatedMinutes: 6,
				ConceptTags:      []string{"scheduler", "dispatch", "response-time"},
				Prerequisites:    prereqList(lessonPrereq),
				Unlocks:          []string{spec.id + ":s2"},
				Config:           cfg,
				Commands:         commands,
			...
			},
			{
				...
				Objective:        "Relate timeline observations to completion behavior.",
				Prompt:           "Explain why each process completes when it does and verify outcome metrics.",
				Difficulty:       "core",
				EstimatedMinutes: 8,
				ConceptTags:      []string{"fairness", "turnaround-time", "workload"},
				Prerequisites:    []string{spec.id + ":s1"},
				Unlocks:          []string{spec.id + ":s3"},
				Config:           cfg,
				Commands:         commands,
				Validators: []ValidatorSpec{
			...
			},
			{
				...
				Objective:        "Apply a policy/config change and verify deterministic behavior remains valid.",
				Prompt:           "Use the tuned scheduler configuration and compare behavior against prior stages.",
				Difficulty:       "advanced",
				EstimatedMinutes: 10,
				ConceptTags:      []string{"policy", "quantum", "determinism"},
		...
		Stages: []Stage{
			{
				...
				Objective:        "Recognize deterministic VA translation and fault events in trace output.",
				Prompt:           "Run the access pattern and observe fault and access trace events.",
				Difficulty:       "intro",
				EstimatedMinutes: 6,
				ConceptTags:      []string{"virtual-memory", "fault", "trace"},
				Prerequisites:    prereqList(lessonPrereq),
				Unlocks:          []string{spec.id + ":s2"},
				Config:           cfg,
				Commands:         commands,
			...
			},
			{
				...
				Objective:        "Connect access sequence to exact not-present fault totals.",
				Prompt:           "Explain why the observed fault count matches this workload.",
				Difficulty:       "core",
				EstimatedMinutes: 9,
				ConceptTags:      []string{"fault-count", "frame-pressure", "determinism"},
				Prerequisites:    []string{spec.id + ":s1"},
				Unlocks:          []string{spec.id + ":s3"},
				Config:           cfg,
				Commands:         commands,
			...
			},
			{
				...
				Objective:        "Apply a memory configuration change and verify pressure improves predictably.",
				Prompt:           "Run with one extra frame and confirm fault pressure does not increase.",
				Difficulty:       "advanced",
				EstimatedMinutes: 10,
				ConceptTags:      []string{"frames", "configuration", "replacement"},
				Prerequisites:    []string{spec.id + ":s2"},
				Config:           applyCfg,
				Commands:         commands,
				Validators: []ValidatorSpec{
		...
		Stages: []Stage{
			{
				...
				Objective:        "Identify deterministic block and wakeup transitions in trace output.",
				Prompt:           "Run the blocking path and observe when work resumes.",
				Difficulty:       "intro",
				EstimatedMinutes: 7,
				ConceptTags:      []string{"blocking", "interrupt", "wakeup"},
				Prerequisites:    prereqList(lessonPrereq),
				Unlocks:          []string{spec.id + ":s2"},
				Config:           cfg,
				Commands:         commands,
			...
			},
			{
				...
				Objective:        "Relate wakeup timing to eventual process completion.",
				Prompt:           "Verify completion and justify it from observed wakeup flow.",
				Difficulty:       "core",
				EstimatedMinutes: 8,
				ConceptTags:      []string{"completion", "io-latency", "scheduling"},
				Prerequisites:    []string{spec.id + ":s1"},
				Unlocks:          []string{spec.id + ":s3"},
				Config:           cfg,
				Commands:         commands,
			...
			},
			{
				...
				Objective:        "Apply a latency configuration change and verify wakeup/completion invariants.",
				Prompt:           "Run with tuned device latency and compare unblock timing.",
				Difficulty:       "advanced",
				EstimatedMinutes: 10,
				ConceptTags:      []string{"latency", "irq", "deterministic-io"},
				Prerequisites:    []string{spec.id + ":s2"},
				Config:           applyCfg,
				Commands:         commands,
				Validators: []ValidatorSpec{
		...
		Stages: []Stage{
			{
				...
				Objective:        "Identify deterministic filesystem path resolution events.",
				Prompt:           "Run the file program and inspect path traversal in trace output.",
				Difficulty:       "intro",
				EstimatedMinutes: 6,
				ConceptTags:      []string{"filesystem", "path-traversal", "inode"},
				Prerequisites:    prereqList(lessonPrereq),
				Unlocks:          []string{spec.id + ":s2"},
				Config:           cfg,
				Commands:         commands,
			...
			},
			{
				...
				Objective:        "Connect file operations to deterministic block mapping events.",
				Prompt:           "Explain how the file operation sequence maps onto blocks.",
				Difficulty:       "core",
				EstimatedMinutes: 8,
				ConceptTags:      []string{"blockmap", "io", "filesystem-trace"},
				Prerequisites:    []string{spec.id + ":s1"},
				Unlocks:          []string{spec.id + ":s3"},
				Config:           cfg,
				Commands:         commands,
			...
			},
			{
				...
				Objective:        "Apply tuned storage settings while preserving filesystem invariants.",
				Prompt:           "Run with adjusted disk latency and verify invariant checks still pass.",
				Difficulty:       "advanced",
				EstimatedMinutes: 9,
				ConceptTags:      []string{"invariants", "consistency", "storage-latency"},
				Prerequisites:    []string{spec.id + ":s2"},
				Config:           applyCfg,
				Commands:         commands,
				Validators: []ValidatorSpec{
